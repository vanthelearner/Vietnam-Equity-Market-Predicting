...
class PreprocessConfig:
    ...
    liquidity_category: str = "core_liquid_top50"
...
def load_config(path: str | Path) -> PipelineConfig:
    ...
    cfg = PipelineConfig(
        ...
        models=ModelConfig(
            ols=_merge(ModelConfig().ols, (raw.get("models") or {}).get("ols")),
            ols3=_merge(ModelConfig().ols3, (raw.get("models") or {}).get("ols3")),
            enet=_merge(ModelConfig().enet, (raw.get("models") or {}).get("enet")),
            pls=_merge(ModelConfig().pls, (raw.get("models") or {}).get("pls")),
            pcr=_merge(ModelConfig().pcr, (raw.get("models") or {}).get("pcr")),
            gbrt=_merge(ModelConfig().gbrt, (raw.get("models") or {}).get("gbrt")),
            rf=_merge(ModelConfig().rf, (raw.get("models") or {}).get("rf")),
            nn=_merge(ModelConfig().nn, (raw.get("models") or {}).get("nn")),
        ),
    )
