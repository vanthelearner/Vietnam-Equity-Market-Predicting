# version_2 Model Pipeline Map

**Purpose**
This pack documents the active model-side pipeline in `version_2`. It starts from the daily processed panel from `/process`, builds a monthly stock panel, filters and transforms features, runs rolling out-of-sample models, builds benchmark-relative and portfolio outputs, and exposes notebook-driven review workflows.

## Linked Notes
- [Model README](01_README.md)
- [Default model config](03_configs_default_yaml.md)
- [careful_v3 config](35_configs_careful_v3_yaml.md)
- [max_v3 config](36_configs_max_v3_yaml.md)
- [Monthly input preparation](11_src_v2_model_prepare_inputs.md)
- [Model preprocessing](12_src_v2_model_preprocess.md)
- [Pipeline orchestrator](17_src_v2_model_pipeline.md)
- [Feature profile registry](34_src_v2_model_feature_profiles.md)
...
It consumes `/process/outputs/03_model_data/daily_model_data.csv` and `/data/risk-free.csv`, writes a monthly model panel into `/model/data`, then trains and evaluates models under `/model/outputs/run_...`.


## Visual Flow
```mermaid
flowchart LR
    A[/process/outputs/03_model_data/daily_model_data.csv/] --> B[prepare_inputs.py]
    RF[/data/risk-free.csv/] --> B
    B --> C[/model/data/panel_input.csv/]
    B --> D[/model/data/benchmark_monthly.csv/]
    C --> E[preprocess.py]
    D --> E
    E --> F[cv.py rolling windows]
    F --> G[pipeline.py]
    G --> H[OLS / OLS3 / ENET / PLS / PCR / GBRT / RF / NN]
    H --> I[/outputs/run_*/predictions/]
    H --> J[/outputs/run_*/portfolio/]
    H --> K[/outputs/run_*/benchmark/]
    H --> L[/outputs/run_*/compare/]
    C --> M[recommend.py]
```
...
| --- | --- | --- |
| `/model/data/panel_input.csv` | `/model/src/v2_model/prepare_inputs.py` | Maximum monthly panel with required schema and Batch 3 optional predictors |
| `/model/data/benchmark_monthly.csv` | `/model/src/v2_model/prepare_inputs.py` | Monthly VN market benchmark series |
| `/model/data/panel_prep_summary.csv` | `/model/src/v2_model/prepare_inputs.py` | High-level monthly panel counts and coverage |
| `/model/data/window_coverage_summary.csv` | `/model/src/v2_model/prepare_inputs.py` | Rolling-window coverage overview |
| `/model/outputs/run_*/...` | `/model/src/v2_model/pipeline.py` | Per-run model outputs, benchmark comparisons, portfolio tables, and comparison summaries |
...
n_assets,699
n_months,270
n_optional_features,131
date_min,2003-10-31 00:00:00
date_max,2026-03-31 00:00:00

metric,value
n_windows,15
first_train_range,2003-10-31 00:00:00 -> 2008-09-30 00:00:00
first_val_range,2008-10-31 00:00:00 -> 2010-09-30 00:00:00
first_test_range,2010-10-31 00:00:00 -> 2011-09-30 00:00:00
test_union_range,2010-10-31 00:00:00 -> 2025-09-30 00:00:00
```
...
- Active notebooks use `build_latest_recommendations(...)`, not the archived true-latest recommendation path.
- `model/not_working/` exists but is not part of the active pipeline.
- Feature-profile selection is an active part of the model pipeline; the process pipeline is profile-agnostic.
