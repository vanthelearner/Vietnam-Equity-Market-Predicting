...
## Purpose
Profile-specific override that switches the model pipeline from the default feature profile to `careful_v3`. Source: `/model/configs/careful_v3.yaml`.

## Where it sits in the pipeline
Used by notebooks or CLI runs when you want the narrower Batch 3 feature profile. It includes `default.yaml` and overrides one preprocessing field.
...
No direct outputs. It changes the optional feature set selected in preprocessing.

## How the code works
The file uses the include mechanism: it inherits all of `default.yaml`, then overrides `preprocess.feature_profile` to `careful_v3`.
...
## Math / logic
$$X_{{careful}} = X[\text{{FEATURE\_PROFILES["careful\_v3"]}}]$$

## Worked Example
If the max panel contains 140 columns, this config tells preprocessing to keep the narrower optional subset. The sample filters stay unchanged; only feature width changes.

## Visual Flow
```mermaid
flowchart LR
    A[careful_v3.yaml] --> B[config.py merge]
    B --> C[PipelineConfig]
    C --> D[preprocess feature_profile = careful_v3]
```
...
- This file does not change liquidity or price/size thresholds.
- It only narrows the optional feature list.
